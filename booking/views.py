...
from .models import Hotels, PaymentStatus, Rooms, Reservation, PaymentStatus, Photo, Category
...
def homepage(request):
    all_location = Hotels.objects.values_list(
        'location', 'id').distinct().order_by()
    if request.method == "POST":
        try:
            print(request.POST)
            hotel = Hotels.objects.all().get(
            ...
            room = Rooms.objects.all().filter(hotel=hotel, capacity__gte=int(
...
def book_room(request):
    ...
    if request.method == "POST":
        ...
        reservation.guest_count = total_person
        reservation.check_in = request.POST['check_in']
        reservation.check_out = request.POST['check_out']
        reservation.payment_status = int(PaymentStatus.Pending)
        reservation.save()
...
def khalti_request(request, b_id):

    booking = Reservation.objects.all().get(id=b_id)
    context = {
        "booking": booking
    }
...
def khalti_verify(request):
    ...
    b_id = request.GET.get("booking_id")
    ...
    if resp_obj.get("idx"):
        success = True
        reservation = Reservation.objects.all().get(id=b_id)
        reservation.payment_status = int(PaymentStatus.Paid)
        reservation.save()
